...
```{r}
chisq.test(table(negSchlagzeileRev))
```
...
PCA durchführen
```{r}
pca.blockF <- principal(blockF, nfactors = 2)
pca.blockF
...
names(blockF.scores) <- c("AngstKriminal", "Kultur")
blockF.scores <- round((blockF.scores),4)
(head(blockF.scores))
```

Wir haben jetzt 2 Komponenten (AngstKriminalität, Kultur) mit je 2 Variablen.
Auswertungen für beide Komponenten müssen gemacht werden.



Shapirotest Normalverteilung und density Plot für Komponente 1
```{r}

shapiro.test(blockF.scores$AngstKriminal)

dH41 <- density(blockF.scores$AngstKriminal) # returns the density data
plot(dH41, main = "FrageblockF - Dichteverteilung")
```

Shapirotest Normalverteilung und density Plot für Komponente 2
```{r}

shapiro.test(blockF.scores$Kultur)

dH42 <- density(blockF.scores$Kultur) # returns the density data
plot(dH42, main = "FrageblockF - Dichteverteilung")
```
...
```{r}
boxplot(blockF.scores$AngstKriminal ~ medienRev$score2)
```
Boxplot Komponente 2
```{r}
boxplot(blockF.scores$Kultur ~ medienRev$score2)
```

Kruskal-Wallis-Test Komponente 1
```{r}
kruskal.test(blockF.scores$AngstKriminal ~ medienRev$score2)
```
H0 wird beibehalten, es besteht kein signifikanter Zusammenhang zwischen Medienkonsum und der Vorurteilsbildung der Leser (Komponente1)

Kruskal-Wallis-Test Komponente 2
```{r}
kruskal.test(blockF.scores$Kultur ~ medienRev$score2)
```
H0 wird beibehalten, es besteht kein signifikanter Zusammenhang zwischen Medienkonsum und der Vorurteilsbildung der Leser (Komponente2)
