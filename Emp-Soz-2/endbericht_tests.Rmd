...
```{r}
blockH <- dataset[,c(37:41)]
print(KMOS(blockH), sort = TRUE)
```

Variable F26Nafri aus der Fragebatterie auschliesen weil sie einen zu niedrigen Wert hat
```{r}
...
names(blockF.scores) <- c("AuswVoruLeser")
...
shapiro.test(blockF.scores$AuswVoruLeser)

dH4 <- density(blockF.scores$AuswVoruLeser) # returns the density data
plot(dH4, main = "FrageblockF - Dichteverteilung")
```
...
Boxplot
```{r}
boxplot(blockF.scores$AuswVoruLeser ~ medienRev$score2)
```
Kruskal-Wallis-Test
```{r}
kruskal.test(blockF.scores$AuswVoruLeser ~ medienRev$score2)
```
